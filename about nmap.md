...
(this prevents the server from repeatedly trying to make the request). In other words, the sequence for scanning an open port looks like this:

![cPzF0kU](https://github.com/schoto/THM-Nmap/assets/69323411/4ec5c630-d18c-4204-9303-eb741732679e)
![11](https://github.com/schoto/THM-Nmap/assets/69323411/305d02ef-cbb4-470e-800c-b361caf78f96)
...
![ABCxAwf](https://github.com/schoto/THM-Nmap/assets/69323411/3236d522-9b00-4b10-a739-f1c28ab56381)

FIN scans (```-sF```) work in an almost identical fashion; however, instead of sending a completely empty packet, a request is sent with the FIN flag (usually used to gracefully close an active connection). Once again, Nmap expects a RST if the port is closed.

![gIzKbEk](https://github.com/schoto/THM-Nmap/assets/69323411/cb617bba-224f-4b0e-887c-86f079850a63)
...
![1234](https://github.com/schoto/THM-Nmap/assets/69323411/129bbe62-293f-4270-a9b2-269a1b8f1952)

The expected response for open ports with these scans is also identical, and is very similar to that of a UDP scan. 
If the port is open then there is no response to the malformed packet. Unfortunately (as with open UDP ports), 
that is also an expected behaviour if the port is protected by a firewall, so NULL, FIN and Xmas scans will only ever identify ports as being open|filtered, 
closed, or filtered. If a port is identified as filtered with one of these scans then it is usually because the target has responded with an ICMP unreachable packet.
...
this is not always the case in practice. In particular Microsoft Windows (and a lot of Cisco network devices) are known to respond with a RST to any malformed TCP packet
-- regardless of whether the port is actually open or not. This results in all ports showing up as being closed.
...
There are a variety of other switches which Nmap considers useful for firewall evasion. We will not go through these in detail.
